...
Por ahora se muestran valores vacíos.
Después se alimentarán desde NotificacionService.
*/
...
<div class="admin-layout">
            ...
            >
                ...
                <div>
                    ...
                    <p class="admin-topbar-subtitle">
                        Gestión de consultorios psicológicos
                    </p>
                </div>
            </div>
    ...
    <span
        ...
        aria-label="<?= (int) $totalNotificacionesNoLeidas ?> notificaciones no leídas"
    >
...
>
    ...
    <?php if (!empty($notificacionesRecientes)): ?>
        ...
        <ul class="admin-notification-list">

            <?php foreach ($notificacionesRecientes as $notificacion): ?>

                <?php
                    $clave = $notificacion['ClvNotif'] ?? '';

                    $esNoLeida =
                        (int) ($notificacion['LeidaNotif'] ?? 0) === 0;

                    $fecha = '';

                    if (!empty($notificacion['FechaNotif'])) {
                        try {
                            $fecha = (new DateTime(
                                $notificacion['FechaNotif']
                            ))->format('d/m/Y h:i A');
                        } catch (Throwable $e) {
                            $fecha = $notificacion['FechaNotif'];
                        }
                    }
                ?>

                <li
                    class="admin-notification-item <?= $esNoLeida ? 'unread' : '' ?>"
                >
