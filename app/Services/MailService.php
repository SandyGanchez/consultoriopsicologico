...
public function enviarRecuperacionConsultorio(
    string $correoDestino,
    string $nombreDestino,
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Restablece el acceso de tu consultorio en PsicoMatch';
        ...
        $urlSegura = htmlspecialchars($urlRecuperacion, ENT_QUOTES, 'UTF-8');
...
Se solicitó restablecer el acceso de tu cuenta. Define una nueva contraseña
con el siguiente enlace. Expira en {$horas} horas y solo puede usarse una vez.
No se incluye ni se cambia tu contraseña en este mensaje.
...
HTML;
        ...
        $mail->AltBody =
            "Hola {$nombreDestino}.\n\n" .
            "Restablece el acceso de tu consultorio en:\n{$urlRecuperacion}\n";

        $mail->send();
        $this->registrarUrlActivacionDesarrollo(
            'recuperacion-consultorio',
            $correoDestino,
            $urlRecuperacion
        );
    } catch (MailException $e) {
        error_log('Error PHPMailer recuperación consultorio: envío fallido');
        throw new RuntimeException(
...
HTML;
    ...
    public function enviarConfirmacionCitaPsicologo(array $contexto): void
    {
        ...
        $this->enviarPlantillaCita(
            (string) ($contexto['CorreoPsicologo'] ?? ''),
            trim((string) ($contexto['NombrePsicologo'] ?? '')),
            'Nueva cita programada — ' . $fecha . ' ' . $hora,
            $this->cuerpoConfirmacionPsicologo($contexto),
            $this->textoConfirmacionPsicologo($contexto)
        );
    }
    ...
    public function enviarRecordatorioCitaPsicologo(array $contexto): void
    {
        $fecha = $this->formatearFechaCita($contexto);
        $hora = $this->formatearHoraCita($contexto);
        ...
        $this->enviarPlantillaCita(
            (string) ($contexto['CorreoPsicologo'] ?? ''),
            trim((string) ($contexto['NombrePsicologo'] ?? '')),
            'Recordatorio de cita — ' . $fecha . ' ' . $hora,
            $this->cuerpoRecordatorioPsicologo($contexto),
            $this->textoRecordatorioPsicologo($contexto)
        );
    }
    ...
    private function enviarPlantillaCita(
        ...
        if ($correo === '' || !filter_var($correo, FILTER_VALIDATE_EMAIL)) {
            throw new RuntimeException('Destinatario de correo no válido.');
        }
    ...
    private function envolverHtmlCita(string $fromNameEsc, string $contenido): string
    {
        return <<<HTML
<!DOCTYPE html>
<html lang="es">
...
<div style="max-width:580px;margin:0 auto;background:#ffffff;border-radius:22px;overflow:hidden;box-shadow:0 12px 40px rgba(101,113,102,.15);">
<div style="padding:28px;background:linear-gradient(135deg,#DAEBE3,#99CDD8);text-align:center;">
<h2 style="margin:0;color:#465149;font-size:23px;">{$fromNameEsc}</h2>
</div>
<div style="padding:35px 36px;">
{$contenido}
</div>
...
HTML;
    ...
    private function formatearFechaCita(array $contexto): string
    {
        $fecha = (string) ($contexto['FechaCita'] ?? '');
        if ($fecha === '') {
            return '';
    ...
    private function formatearHoraCita(array $contexto): string
    {
        return substr((string) ($contexto['HraInicioCita'] ?? ''), 0, 5);
    }

    private function bloqueDatosOperativos(array $contexto, bool $paraPaciente): string
    {
        $esc = static fn(string $v): string => htmlspecialchars($v, ENT_QUOTES, 'UTF-8');
        $fecha = $esc($this->formatearFechaCita($contexto));
        $hora = $esc($this->formatearHoraCita($contexto));
        ...
        $lineas = '';
        if ($paraPaciente) {
            $psi = $esc(trim((string) ($contexto['NombrePsicologo'] ?? '')));
            $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Especialista:</strong> {$psi}</p>";
        } else {
            $pac = $esc(trim((string) ($contexto['NombrePaciente'] ?? '')));
            $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Paciente:</strong> {$pac}</p>";
        }

        $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Servicio:</strong> {$servicio}</p>";
        $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Fecha:</strong> {$fecha}</p>";
        $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Hora:</strong> {$hora}</p>";
        $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Duración:</strong> {$duracion} minutos</p>";
        $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Consultorio:</strong> {$consultorio}</p>";

        if ($paraPaciente) {
            $costo = number_format((float) ($contexto['CostoAplicado'] ?? 0), 2, '.', ',');
            $dir = $esc(trim((string) ($contexto['DireccionCompleta'] ?? '')));
            $tel = $esc(trim((string) ($contexto['TelefonoCons'] ?? '')));
            $correoCons = $esc(trim((string) ($contexto['CorreoConsultorio'] ?? '')));
            $cancel = $esc(trim((string) ($contexto['TextoCancelacion'] ?? '')));

            $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Costo confirmado:</strong> \${$costo}</p>";
            if ($dir !== '') {
                $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Dirección:</strong> {$dir}</p>";
            }
            if ($tel !== '') {
                $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Teléfono:</strong> {$tel}</p>";
            }
            if ($correoCons !== '') {
                $lineas .= "<p style=\"margin:0 0 8px;\"><strong>Correo de contacto:</strong> {$correoCons}</p>";
            }
            if ($cancel !== '') {
                $lineas .= "<p style=\"margin:0;\"><strong>Cancelación:</strong> {$cancel}</p>";
            }
        }

        return '<div style="margin:28px 0;padding:22px;border-radius:18px;background:#FDE8D3;">'
            . $lineas
            . '</div>';
    }
...
HTML;
    ...
    {
        $esc = static fn(string $v): string => htmlspecialchars($v, ENT_QUOTES, 'UTF-8');
        $nombre = $esc(trim((string) ($contexto['NombrePsicologo'] ?? '')));
        $url = $esc((string) ($contexto['UrlAgendaPsicologo'] ?? ''));
        $bloque = $this->bloqueDatosOperativos($contexto, false);

        return <<<HTML
<p>Hola <strong>{$nombre}</strong>:</p>
<p style="line-height:1.7;">Se registró una nueva cita con estado <strong>Programada</strong>.</p>
{$bloque}
<div style="margin-top:28px;text-align:center;">
<a href="{$url}" style="display:inline-block;padding:13px 24px;border-radius:14px;background:#99CDD8;color:#465149;font-weight:bold;text-decoration:none;">Abrir mi agenda</a>
</div>
HTML;
    }

    private function textoConfirmacionPsicologo(array $contexto): string
    {
        return "Hola " . trim((string) ($contexto['NombrePsicologo'] ?? '')) . ".\n\n"
            . "Nueva cita programada.\n"
            . "Paciente: " . trim((string) ($contexto['NombrePaciente'] ?? '')) . "\n"
            . "Servicio: " . trim((string) ($contexto['NombreServicio'] ?? '')) . "\n"
            . "Fecha: " . $this->formatearFechaCita($contexto) . " "
            . $this->formatearHoraCita($contexto) . "\n"
            . "Agenda: " . (string) ($contexto['UrlAgendaPsicologo'] ?? '') . "\n";
    }

    private function cuerpoRecordatorioPaciente(array $contexto): string
    {
        $esc = static fn(string $v): string => htmlspecialchars($v, ENT_QUOTES, 'UTF-8');
        $nombre = $esc(trim((string) ($contexto['NombrePaciente'] ?? '')));
        $url = $esc((string) ($contexto['UrlMisCitas'] ?? $contexto['UrlLogin'] ?? ''));
        $bloque = $this->bloqueDatosOperativos($contexto, true);

        return <<<HTML
<p>Hola <strong>{$nombre}</strong>:</p>
<p style="line-height:1.7;">Te recordamos que tienes una cita programada.</p>
{$bloque}
<div style="margin-top:28px;text-align:center;">
<a href="{$url}" style="display:inline-block;padding:13px 24px;border-radius:14px;background:#99CDD8;color:#465149;font-weight:bold;text-decoration:none;">Ver mis citas</a>
</div>
HTML;
    }

    private function textoRecordatorioPaciente(array $contexto): string
    {
        return "Hola " . trim((string) ($contexto['NombrePaciente'] ?? '')) . ".\n\n"
    ...
    private function cuerpoRecordatorioPsicologo(array $contexto): string
    {
        $esc = static fn(string $v): string => htmlspecialchars($v, ENT_QUOTES, 'UTF-8');
        $nombre = $esc(trim((string) ($contexto['NombrePsicologo'] ?? '')));
        $url = $esc((string) ($contexto['UrlAgendaPsicologo'] ?? ''));
...
HTML;
    ...
    private function textoRecordatorioPsicologo(array $contexto): string
    {
        return "Hola " . trim((string) ($contexto['NombrePsicologo'] ?? '')) . ".\n\n"
            . "Te recordamos que tienes una cita programada.\n"
            . "Paciente: " . trim((string) ($contexto['NombrePaciente'] ?? '')) . "\n"
