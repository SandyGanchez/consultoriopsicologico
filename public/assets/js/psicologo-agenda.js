...
document.addEventListener('DOMContentLoaded', () => {
    if (window.__psiAgendaJsInit) {
        return;
    }
    window.__psiAgendaJsInit = true;

    iniciarCalendarioPsicologo();
    iniciarModalNuevaCita();
    iniciarAsistenciaCitaPsi();
    iniciarRefrescoPendientesAgenda();
});

let contextoDetalleCitaPsi = null;
let enviandoAsistenciaPsi = false;
let timerIntervaloPendientesAgenda = null;
let timerProximaCitaAgenda = null;
let peticionPendientesEnCurso = false;
let secuenciaPendientesAgenda = 0;
let abortPendientesAgenda = null;
let pendientesAgendaActivo = true;
let avisoSesionPendientesMostrado = false;
let pendientesAgendaInit = false;
...
function obtenerUrlPendientesAgenda() {
    return String(obtenerConfigAgenda().pendientesUrl || '');
}

function obtenerIntervaloPendientesMs() {
    return Math.max(
        60000,
        Number(obtenerConfigAgenda().refrescoPendientesMs) || 60000
    );
}

function urlAccionOperativaPermitida(urlCruda) {
...
function programarProximaEvaluacionAgenda(isoProxima, isoAhoraServidor) {
    if (timerProximaCitaAgenda) {
        clearTimeout(timerProximaCitaAgenda);
        timerProximaCitaAgenda = null;
    }

    if (!pendientesAgendaActivo || !isoProxima || !isoAhoraServidor) {
        return;
    }
    ...
    // Espera relativa al reloj del servidor.
    const espera = Math.max(1000, objetivo - ahoraSrv + 250);
    timerProximaCitaAgenda = setTimeout(() => {
        refrescarPendientesAgenda({ forzarRefetch: true });
    }, espera);
}

function detenerPendientesAgenda() {
    pendientesAgendaActivo = false;

    if (timerIntervaloPendientesAgenda) {
        clearInterval(timerIntervaloPendientesAgenda);
        timerIntervaloPendientesAgenda = null;
    }
    ...
    if (abortPendientesAgenda) {
        try {
            abortPendientesAgenda.abort();
        } catch (e) {
            // ignore
        }
        abortPendientesAgenda = null;
    }

    peticionPendientesEnCurso = false;
}

function avisarSesionPendientesAgenda() {
    if (avisoSesionPendientesMostrado) {
        return;
    }

    avisoSesionPendientesMostrado = true;
    detenerPendientesAgenda();

    const loginUrl = String(obtenerConfigAgenda().loginUrl || '');
    const mensaje =
    ...
    let vivo = document.getElementById('psiAgendaSesionAviso');
    if (!vivo) {
        const host =
            document.getElementById('calendarioPsicologo')?.parentElement ||
            document.querySelector('.psicologo-agenda-page') ||
            document.body;
        vivo = document.createElement('div');
        vivo.id = 'psiAgendaSesionAviso';
        vivo.className = 'alert alert-warning mt-3';
...
function aplicarEstadoOperativoAEvento(evento, item) {
    if (!evento || !item) {
        return;
    }
    ...
    evento.setExtendedProp('etiquetaClinica', item.etiquetaClinica || '');
    evento.setExtendedProp(
        'urlClinica',
        urlAccionOperativaPermitida(item.urlClinica || '')
    );
    evento.setExtendedProp(
        'urlVerPaciente',
        urlAccionOperativaPermitida(item.urlVerPaciente || '')
    );
    evento.setExtendedProp(
        'urlCompletarDatos',
        urlAccionOperativaPermitida(item.urlCompletarDatos || '')
    );

    if (item.estadoCita) {
        evento.setExtendedProp('estado', item.estadoCita);
    }
}

function aplicarSnapshotACalendario(datos) {
    const calendar = contextoAgenda.calendar;
    if (!calendar || !datos || !Array.isArray(datos.citas)) {
        return;
    }

    const mapa = new Map(
        datos.citas.map((item) => [String(item.clvCita || ''), item])
    );

    calendar.getEvents().forEach((evento) => {
        const id = String(evento.extendedProps?.clvCita || evento.id || '');
        const item = mapa.get(id);
        if (item) {
            aplicarEstadoOperativoAEvento(evento, item);
        }
    });

    if (
        contextoDetalleCitaPsi?.clvCita &&
        mapa.has(String(contextoDetalleCitaPsi.clvCita))
    ) {
    ...
    programarProximaEvaluacionAgenda(
        datos.proximaEvaluacionIso || null,
        datos.ahora || null
    );
}

function sincronizarContextoDetalleDesdeItem(item) {
    if (!contextoDetalleCitaPsi || !item) {
        return;
    }
    ...
    contextoDetalleCitaPsi.etiquetaClinica = item.etiquetaClinica || '';
    contextoDetalleCitaPsi.urlClinica = urlAccionOperativaPermitida(
        item.urlClinica || ''
    );
    contextoDetalleCitaPsi.urlVerPaciente = urlAccionOperativaPermitida(
        item.urlVerPaciente || ''
    );
    contextoDetalleCitaPsi.urlCompletarDatos = urlAccionOperativaPermitida(
        item.urlCompletarDatos || ''
    );
}

async function refrescarPendientesAgenda(opciones = {}) {
    const url = obtenerUrlPendientesAgenda();
    if (!url || !pendientesAgendaActivo || peticionPendientesEnCurso) {
        return null;
    }

    peticionPendientesEnCurso = true;
    const miSecuencia = ++secuenciaPendientesAgenda;
    abortPendientesAgenda = new AbortController();
    const signal = abortPendientesAgenda.signal;
    ...
    try {
        ...
        if (opciones.clvCita) {
            endpoint +=
                (url.includes('?') ? '&' : '?') +
                'clvCita=' +
                encodeURIComponent(String(opciones.clvCita));
        }
        ...
        if (miSecuencia !== secuenciaPendientesAgenda) {
            return null;
        }
        ...
        if (respuesta.status === 401 || respuesta.status === 403) {
            ...
            avisarSesionPendientesAgenda();
            return null;
        }

        if (!respuesta.ok) {
            // Error temporal: no borrar estado ya mostrado.
            return null;
        }
        ...
        aplicarSnapshotACalendario(datos);

        if (opciones.forzarRefetch) {
        ...
        // Fallo de red: conservar estado actual.
        return null;
    } finally {
        if (miSecuencia === secuenciaPendientesAgenda) {
            peticionPendientesEnCurso = false;
...
function iniciarRefrescoPendientesAgenda() {
    if (pendientesAgendaInit) {
        return;
    }
    ...
    pendientesAgendaInit = true;
    const intervalo = obtenerIntervaloPendientesMs();
    ...
    timerIntervaloPendientesAgenda = setInterval(() => {
        if (pendientesAgendaActivo) {
            refrescarPendientesAgenda({ forzarRefetch: false });
        }
    }, intervalo);

    refrescarPendientesAgenda({ forzarRefetch: false });

    window.addEventListener('pagehide', detenerPendientesAgenda);
    window.addEventListener('beforeunload', detenerPendientesAgenda);
}
...
async function mostrarDetalleCitaPsi(evento) {
    const props = evento.extendedProps ?? {};
    const estado = String(props.estado || '').toUpperCase();
    const clvCita = props.clvCita || evento.id || '';
    ...
    contextoDetalleCitaPsi = {
        ...
        urlVerPaciente: props.urlVerPaciente || '',
        urlCompletarDatos: props.urlCompletarDatos || '',
        fechaTexto: formatearFechaPsi(evento.start),
        horarioTexto: formatearHorarioPsi(evento.start, evento.end)
    };

    // Al abrir el modal, refrescar estado operativo de esa cita.
    if (clvCita) {
        const datos = await refrescarPendientesAgenda({ clvCita });
        const item = Array.isArray(datos?.citas)
            ? datos.citas.find(
                  (c) => String(c.clvCita) === String(clvCita)
              )
            : null;
...
function actualizarBloquesAsistenciaPsi(ctx) {
    ...
    const enlaceVerPaciente = document.getElementById(
        'enlaceVerPacienteAsistenciaPsi'
    );
    const enlaceCompletarDatos = document.getElementById(
        'enlaceCompletarDatosAsistenciaPsi'
    );

    enlaceVerPaciente?.classList.add('d-none');
...
async function enviarRegistroAsistenciaPsi(accion) {
    ...
    try {
        ...
        contextoDetalleCitaPsi.urlVerPaciente =
            datos.urlVerPaciente || '';
        contextoDetalleCitaPsi.urlCompletarDatos =
