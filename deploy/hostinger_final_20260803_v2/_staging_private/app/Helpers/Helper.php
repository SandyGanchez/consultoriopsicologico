...
class Helper
{
    ...
    /**
     * Formato monetario operativo MXN (no implica pago cobrado).
     */
    public static function formatearMonedaMxn(
        float|int|string|null $monto
    ): string {
    ...
    /**
     * Dirección postal completa desde una fila direccion (fuente única).
     * Omitir vacíos; ReferenciaDir va aparte.
     *
     * @param array<string, mixed> $datos
     */
    public static function construirDireccionCompleta(array $datos): string
    {
        $segmentos = [];
        ...
        if ($calle !== '' || $numExt !== '' || $numInt !== '') {
            $lineaCalle = $calle;

            if ($numExt !== '') {
                $lineaCalle = trim($lineaCalle . ' ' . $numExt);
            }
        ...
        if ($colonia !== '') {
            $segmentos[] = (stripos($colonia, 'col') === 0)
                ? $colonia
                : 'Col. ' . $colonia;
        }
    ...
    /**
     * Alias público: misma construcción que construirDireccionCompleta.
     *
     * @param array<string, mixed> $datos
     */
    public static function direccionPublicaLegible(array $datos): string
    {
        return self::construirDireccionCompleta($datos);
    }

    public static function direccionPublicaResumida(array $datos): string
    {
        $completa = self::construirDireccionCompleta($datos);
    ...
    /**
     * Normaliza lat/lng: coma→punto, rechaza NaN/Infinity/texto.
     */
    public static function normalizarCoordenada(mixed $valor): ?float
    {
        if ($valor === null) {
            return null;
        }
    ...
    public static function coordenadasPublicasValidas(
        ...
        $lat = self::normalizarCoordenada($latitud);
        $lng = self::normalizarCoordenada($longitud);
    ...
    /**
     * Enlace “Cómo llegar” con coordenadas reales o fallback a dirección.
     */
    public static function enlaceComoLlegar(
        mixed $latitud,
        mixed $longitud,
        string $direccionFallback = ''
    ): string {
        if (self::coordenadasPublicasValidas($latitud, $longitud)) {
            $lat = self::normalizarCoordenada($latitud);
            $lng = self::normalizarCoordenada($longitud);
        ...
        $direccionFallback = trim($direccionFallback);

        if ($direccionFallback === '') {
            return '';
        }

        return 'https://www.google.com/maps/search/?api=1&query='
            . rawurlencode($direccionFallback);
    }
