...
public function obtenerServiciosDisponibles(
    ...
    public function obtenerActivosParaAgendamiento(): array
    {
        $sql = "SELECT
                    psi.ClvPsi,
                    psi.ClvCons,
    ...
    public function obtenerParaAgendamiento(
        string $clvPsi
    ): ?array {
        $sql = "SELECT
                    psi.ClvPsi,
    ...
    public function guardar(
        ...
        $this->guardarPendienteActivacion($datos, $clvCons);
    }

    /**
     * Crea persona, usuario inactivo y psicólogo fuera de página pública.
     * Respeta una transacción externa (no hace commit propio si ya hay una).
     *
     * @return array{ClvUsu: string, ClvPsi: string, ClvPer: string}
     */
    public function guardarPendienteActivacion(
        array $datos,
        string $clvCons
    ): array {
        ...
        try {
            ...
            (new \App\Services\ServicioOfertaService($this->db))
                ->incorporarServiciosActivosAPsicologo(
                    $clvPsi,
                    $clvCons
                );
    ...
    /**
     * Activa al psicólogo tras establecer contraseña (mantiene oculto en página).
     *
     * @return array{ClvPsi: string, ClvCons: string}
     */
    public function activarTrasInvitacion(string $clvUsu): array
    {
        $sql = "SELECT ClvPsi, ClvCons
                FROM psicologo
                WHERE ClvUsu = :clvUsu
                LIMIT 1
                FOR UPDATE";
        ...
        $psi = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        $update->execute(['clvUsu' => trim($clvUsu)]);
