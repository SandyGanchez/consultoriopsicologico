...
{
  /**
   * Autenticación única para todos los roles.
   *
   * @return array{ok: bool, mensaje?: string, usuario?: array<string, mixed>}
   */
  public function autenticar(
    ...
    $hash = (string) ($usuario['ContrasenaUsu'] ?? '');
    ...
    $estadoUsuario = (int) ($usuario['EstadoUsu'] ?? 0);

    if ($estadoUsuario !== 1) {
        $requiereCambio = (int) ($usuario['RequiereCambioContrasena'] ?? 0) === 1;
        $mensajeInactiva = $requiereCambio
            ? 'Tu cuenta aún no está activa. Revisa tu correo de activación o solicita un nuevo enlace.'
            : AccesoSesionService::MENSAJE_INACTIVA;

        return [
            'ok' => false,
            'mensaje' => $mensajeInactiva
        ];
    }
  ...
  public function login(
    ...
    $resultado = $this->autenticar($correo, $password);
    ...
    public function registrar(array $datos): void
    {
        ...
        $validacionConsentimiento = $privacidad->validarCheckboxesConsentimiento(
            $datos
        );

        if (!$validacionConsentimiento['ok']) {
            Session::set(
                'error',
                (string) ($validacionConsentimiento['mensaje'] ??
        ...
        if (!$privacidad->persistenciaDisponible()) {
            Session::set(
                'error',
                'El registro de consentimiento no está disponible. Contacta al consultorio.'
            );
            Response::redirect('registro');
        }
